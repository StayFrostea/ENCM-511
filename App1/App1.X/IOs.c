...
void __attribute__((interrupt, no_auto_psv))_CNInterrupt(void) {
    ...
    if (IFS1bits.CNIF == 1) {
        ...
        if (PORTAbits.RA2 == 0 && PORTAbits.RA4 == 1 && PORTBbits.RB4 == 1) {
            ...
            c = 2;
        } else if ((PORTAbits.RA2 == 1 && PORTAbits.RA4 == 1 && PORTBbits.RB4 == 1) && (time==0) && (runFlag==0)) {
            c = 3;
        } else if ((PORTAbits.RA2 == 1 && PORTAbits.RA4 == 1 && PORTBbits.RB4 == 1) && (time>0) && (runFlag==0)) {
            c = 4;
        } else if ((PORTAbits.RA2 == 1 && PORTAbits.RA4 == 1 && PORTBbits.RB4 == 1) && (time==0) && (runFlag==1)) {
            c = 5;
        } else if ((PORTAbits.RA2 == 1 && PORTAbits.RA4 == 1 && PORTBbits.RB4 == 1) && (time>0) && (runFlag==1)) {
            c = 6;
        }
...
void IOcheck(void) {
    switch (c) {
        ...
        case 2: //only PB3 is pressed
            ...
            NewClk(32); // slow down clock for delay and other tasks
            break;
        case 3: //No buttons, time = 0, not running
            NewClk(8);
            Disp2String("\r"); // print time to terminal
            Disp2Dec(time / 60 * 1000);
            Disp2String("m : ");
            ...
            NewClk(32); // slow down clock for delay and other tasks
            break;
        case 4: //No buttons, time !=0, not running
            NewClk(8);
            Disp2String("\r"); // print time to terminal
            Disp2Dec(time / 60 * 1000);
            Disp2String("m : ");
            ...
            NewClk(32); // slow down clock for delay and other tasks
            break;
        case 5: //No buttons, time = 0, running, alarm
            NewClk(8);
            Disp2String("\r"); // print time to terminal
            Disp2Dec(time / 60 * 1000);
            Disp2String("m : ");
            ...
            NewClk(32); // slow down clock for delay and other tasks
            break;
        case 6: //No buttons, time !=0, running
            NewClk(8);
            Disp2String("\r"); // print time to terminal
            Disp2Dec(time / 60 * 1000);
            Disp2String("m : ");
